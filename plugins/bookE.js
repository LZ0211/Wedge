...
function Encode(book){
    ...
    //MAGIC
    datas.push(new Buffer("NEBK"));

    //META
    var infos = ["title","author","cover","classes","brief","date","uuid"];
    book.meta = book.meta || {};
    infos.forEach(function (info){
        book.meta[info] = book.meta[info] || "";
        var size = new Buffer(4);
        size.writeInt32BE(book.meta[info].length);
        datas.push(size);
        datas.push(new Buffer(book.meta[info]));
    });

    //LIST
    book.list = book.list || [];

    var index = zlib.deflateSync(new Buffer(book.list.map(chapter=>chapter.title.replace(/\n/g," ")).join("\n")));
    var size = new Buffer(4);
    size.writeInt32BE(index.length);
    datas.push(size);
    datas.push(index);

    //CONTENT
    book.list.forEach(function (chapter){
        var content = new Buffer(chapter.content);
        content = zlib.deflateSync(content);
        var crc = new Buffer(4);
        crc.writeInt32BE(crc32(content));
        var length = new Buffer(4);
        length.writeInt32BE(content.length);
        datas.push(length);
        datas.push(content);
        datas.push(crc);
    });
    datas.push(new Buffer("END"));
...
var buffer = fs.readFileSync("book.ebk");
fs.writeFile("book.json",JSON.stringify(Decode(buffer),null,2));
