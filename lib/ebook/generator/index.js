...
function execute(msg,callback){
    ...
    fs.readFile(bookfile,(err,data)=>{
        ...
        //分部生成文件 或 加载完一次性生成
        book = {meta:meta,list:[],location:bookdir};
        sort(list);
        if(_sort == -1){
            list.reverse();
        }
        ...
        let stepFun = (item,next)=>{
            ...
            fs.readFile(file,(err,data)=>{
                ...
                try{
                    ...
                    var img_path = path.join(bookdir,item.id);
                    if (fs.existsSync(img_path)){
                        var images = fs.readdirSync(img_path);
                        if(images.length){
                            item.images = images.map(image=>path.join(img_path,image));
                        }
                    }
                }catch (e){
                    return next();
...
// execute({
//     bookdir:'C:/Users/WangC/Desktop/Downloader/library/3b691a76-4ea2-b737-17b5-c27fa39918d2',
//     formation:'epub',
//     directory:'./'
// },console.log)