...
const Convertor = {
    ...
    wbk2html:filename=>Ebk.unwbk(readFile(filename),data=>Ebk.html(data,svaeFile(filename,'html'))),
    ...
    json2html:filename=>Ebk.html(JSON.parse(readFile(filename).toString()),svaeFile(filename,'html')),
    ...
    db2html:filename=>Ebk.html(readDB(filename),svaeFile(filename,'html')),
};
...
if(argv.length > 3){
    ...
    process.on("message",options=>{
        try{
            Ebk.unwbk(fs.readFileSync(options.filename),book=>{
                encodeBook(book);
                var filename = options.filename.replace('.wbk','.'+options.formation);
                Ebk[options.formation](book,data=>{
                    fs.writeFileSync(filename,data);
