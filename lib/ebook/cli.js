...
function commander(msg){
    ...
    if(!generator){
        ...
        directory = path.join(directory, bookData.meta.author + " - " + bookData.meta.title);
        fs.mkdirsSync(directory);
        bookData.list.forEach(chapter=>{
            try{
                var data = fs.readFileSync(path.join(bookdir,chapter.id + '.json'));
                var content = Book.Chapter.set(JSON.parse(data.toString())).valueOf().content.replace(/\n/g,'\r\n');
