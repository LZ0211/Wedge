...
Template.opf = function (book){
    var classes = book.meta.get('classes'),
        ...
        source = book.meta.get('source'),
        uuid = book.meta.get('uuid'),
        id = uuid.replace(/\-/g,''),
        date = book.meta.date.format('yyyy-MM-dd hh:mm:ss'),
        brief = book.meta.get('brief');
    var items = [],
        itemrefs = [],
        references = [];
    book.list.each(chapter=>{
        var id = chapter.get('id');
...
module.exports = function (book,fn){
    ...
    html(book,function (files){
        ...
        book.list.each(function (chapter,index){
