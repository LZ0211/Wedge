...
const SIGNATURE = Buffer.from([0x04,0x03,0x4b,0x50]);
const VERSION = Buffer.from([0x00,0x14]);
...
function getLastModification(){
    var number = new Date()/1000+156476400;
    var date = new Buffer(2);
    var time = new Buffer(2);
    date.writeInt16LE(number/86400);
    time.writeInt16LE(number - 86400 * date.readInt16LE());
    return [time,date];
}
...
function decoder(buffer){
    ...
    function readEOCD(){
        ...
        ending.centralOffset = bin.readInt32LE();
        ending.commentLength = bin.readInt16LE();
        ending.comment = bin.read(ending.commentLength).toString();
        return ending;
    }
    var header = readHeader();
    var fileData = readFileData(header);
    var dataDescriptor = readDataDescriptor(header);
    bin.seek(-22)
    //console.log(readCentralDirectory());
    console.log(readEOCD())
    //console.log(zlib.inflateRawSync(fileData).toString())
}

var zipdata = fs.readFileSync('Wedge.zip');
decoder(zipdata)