...
var word = {
    ...
    document(book){
        ...
        xml += book.list.map((chapter,index)=>(
            `<w:p><w:bookmarkStart w:id="${index+1}" w:name="${leftPad(index+1,'0',5)}"/><w:pPr><w:pStyle w:val="Heading 1"/><w:pageBreakBefore w:val="on"/></w:pPr><w:r><w:t>【龙戏双娇】（第一章 初见少妇）</w:t></w:r><w:bookmarkEnd w:id="${index+1}"/></w:p>` + encode(chapter.content).split('\n').map(line=>`<w:p><w:pPr><w:pStyle w:val="Normal"/></w:pPr><w:r><w:t>${line}</w:t></w:r></w:p>`).join('')
        )).join('');
