...
function Zip(){
    ...
    function generateFile(name,data,uncompress){
        name = Buffer.from(name);
        data = Buffer.from(data);
        var now = new Date();
        var time = dosTime(now);
        var date = dosDate(now);
        var compressedData = uncompress ?  data : zlib.deflateRawSync(data);
        var crc32 = gerCRC32(data);
        var bin = new Bin(Buffer.alloc(30));
        bin.write(LOCAL_FILE_HEADER_SIGNATURE);//4
        bin.write(VERSION);//6
        bin.write(BIT_FLAG);//8
        bin.writeInt16LE(uncompress ? 0 : 8);//10
        bin.writeUInt16LE(time);//12
        bin.writeUInt16LE(date);//14
        bin.write(crc32);//18
        ...
        central.writeInt16LE(uncompress ? 0 : 8);//12
        central.writeUInt16LE(time);//14
        central.writeUInt16LE(date);//16
        ...
        decode(buffer){
            ...
            for(var i=0;i<length;i++){
                ...
                if(header.compressionMethod){
                    ...
                    archiver.append(header.fileName,fileData);
                }
            }
            return archiver;
