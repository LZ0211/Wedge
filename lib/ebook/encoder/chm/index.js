...
function writeHeader(book){
    return Buffer.concat([
        Buffer.from('ITSF'),
        Buffer.from([0x03, 0x00, 0x00, 0x00, 0x60, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00]),
        Binary(Buffer.alloc(4)).writeInt32BE(book.meta.date/1000).rawBuffer,
        Buffer.from([0x04, 0x08, 0x00, 0x00]),
        Buffer.from(book.meta.uuid,'hex'),
        Buffer.from(book.meta.uuid,'hex'),
        Buffer.from([0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
    ]);
}
