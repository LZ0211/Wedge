...
module.exports = function(dir){
    ...
    function render(chapter,next){
        ...
        if(chapter.images){
            fs.mkdirsSync(path.join(temp,chapter.id));
            chapter.images.forEach(image=>{
                var dest = `${temp}/${chapter.id}/${path.basename(image)}`;
                fs.copyFileSync(image,dest);
                images += `${chapter.id}/${path.basename(image)}\n`;
            });
        }
    ...
    return {
        ...
        meta:function(book,next){
            ...
            fs.writeFileSync(path.join(temp,"book.hhp"),iconv.encode(convert.hhp(book) + images,'gbk'));
