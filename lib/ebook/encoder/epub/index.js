...
Template.opf = function (book){
    var classes = book.meta.classes,
        ...
        uuid = book.meta.uuid,
        id = uuid.replace(/\-/g,''),
        date = formatTime(book.meta.date,'yyyy-MM-ddThh:mm:ss'),
    ...
    book.list.forEach(chapter=>{
        ...
        items.push('    <item href="' + id + '.html" id="Chapter_' + id + '" media-type="application/xhtml+xml"/>');
...
Template.ncx = function (book){
    var classes = book.meta.classes,
        ...
        brief = book.meta.brief;
    var navPoints = [];
    book.list.forEach((chapter,idx)=>{
        var id = chapter.id;
        var title = chapter.title;
        navPoints.push('    <navPoint class="chapter" id="Chapter_' + id + '" playOrder="' + (idx+2) + '">')
