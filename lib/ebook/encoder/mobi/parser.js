...
class MOBI{
    ...
    parseMobiHeader(){
        ...
        this.mobiHeader = {
            ...
            extraRecordDataFlags:this.stream.readInt32BE(),
...
function sizeOfTrailingDataEntry(data){
    var num = 0, len = data.length, pos = len - 4,val;
...
function trimTrailingDataEntries(rawdata){
    var data = rawdata;
    for(var i=0;i<trailers;i++){
        var num = sizeOfTrailingDataEntry(data);
        data = data.slice(0,data.length - num);
    }
    console.log(rawdata.slice(data.length))
    num = (data[data.length - 1] & 3) + 1;
    return data.slice(0,data.length - num);
}


var mobi = new MOBI('mobi7-book.mobi');
mobi.read();
mobi.parse();
//console.log(mobi);
var flags = mobi.mobiHeader.extraRecordDataFlags;
var trailers = 0;
while(flags > 1){
    if(flags & 2){
        trailers += 1;
...
var arr = []
for(var i=1;i<200;i++){
    arr.push(LZ77.decompress(trimTrailingDataEntries(mobi.readRecord(i))))
    //LZ77.decompress(trimTrailingDataEntries(mobi.readRecord(10)))
}
console.log(Buffer.concat(arr).toString())
//console.log(LZ77.decompress(trimTrailingDataEntries(mobi.readRecord(10))).toString())