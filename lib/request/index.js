...
function parse(res,callback){
    ...
    function fn(err,res,data){
        if(err) return callback(err,res,data);
        if(utils.isJSON(mime)){
            ...
            if(charset){
                try{
                    charset = charset.split('=').pop().trim().toLowerCase();
                    data = iconv.decode(data,charset);
                }catch(e){}
            }
        }else if(utils.isImage(mime)){
            if(data.length < length) return callback(new Error('Image Corrupt!'),res,null);
        }
    ...
    res.on('end',function(){
        ...
        if(/deflate/i.test(encoding)){
            zlib.inflate(data,function(err,buf){
                if(!err) return fn(null,res,buf);
                zlib.gunzip(data,function(_err,buf){
                    if(!_err) return fn(null,res,buf);
                    return fn(err,res,data);
                });
            });
        }else if(/gzip/i.test(encoding)){
            zlib.gunzip(data,function(err,buf){
                if(!err) return fn(null,res,buf);
                zlib.inflate(data,function(_err,buf){
                    if(!_err) return fn(null,res,buf);
                    return fn(err,res,data);
                });
            });
...
Proto.retry = Proto.reconnect = function (times){
...
Proto.auth = function(user, pass){
    var str = Buffer.from(user + ':' + pass).toString('base64');
...
Proto.proxy = function (host,port){
    ...
    if (!port && host.match(':')){
        var arr = host.split(':')
        this._proxy = {};
        this._proxy.port = arr.pop();
        this._proxy.host = arr.join(":")
        return this;
    }
    this._proxy = {
        host:host,
...
Proto.end = function (callback){
    ...
    function fn(err,res,buffer){
        self.buffer = buffer;
        self.emit('end');
        err ? self.callback(err,res,buffer) : self.callback(null, res, buffer);
    }
    req.on('response',function (res){
        self.res = res;
        var redirect = utils.isRedirect(res.statusCode);
...
function download(url,file){
    ...
    head(url).end(function (err,res,buf){
        ...
        var limit = 1024 * 1024;
        var buffer = Buffer.alloc(limit).fill(0x00);
        if (!size) return;
