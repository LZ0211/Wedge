    ...
    let object = {
        ...
        loadChapterContent:function (fn){
            ...
            Thread().use((item,next)=>{
                ...
                Indexs = Indexs.filter(item=>item.content);
                return fn()
            }).threads(threads).queue(Indexs).start();
        },
        ...
        reOrganize:function(fn){
            object.loadChapterContent(()=>{
                Indexs.sort((a,b)=>a.id - b.id);
                let regexp = /^(第\s*[0-9零一二三四五六七八九十百千万○〇１２３４５６７８９０]{1,8}\s*[章回卷节折篇幕集部].{0,30}|章[0-9零一二三四五六七八九十百千万○〇１２３４５６７８９０]{1,8}.{0,30})$/g
                let content = Indexs.map(item=>item.content).join('');
            });
        },
        valueOf:function (){
            return {
