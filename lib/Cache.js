...
function Cache(byteLimit){
    ...
    function toBuffer(data){
        ...
        if (typeof data === 'boolean'){
            if (data === true){
                boolean = Buffer.from([0x01]);
            }else {
                boolean = Buffer.from([0x00]);
            }
            return {
                type:'boolean',
                length:1,
        ...
        if (typeof data === 'object'){
            ...
            var object = Buffer.from(JSON.stringify(data));
            return {
                type:'object',
    ...
    function set(label,data,forever){
